...
void Game::generateContacts()
{
	...
	collData.reset(maxContacts);
	collData.friction = 0.9;
	collData.tolerance = 0.1;
	collData.restitution = 0.1;

	if(!objects.empty())
	{
		for(unsigned int i = 0; i < objects.size(); i++)
		{
            if(!collData.anyContactsLeft())
            {
                return;
            }

			wind::CollisionDetection::BoxAndHalfSpace(*objects.at(i), *planes.at(0), collData);
